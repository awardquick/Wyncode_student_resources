...
cd !$
```
$! is the [PID](https://en.wikipedia.org/wiki/Process_identifier) of the most recent background command.
...
Generate a `SiteController` with a `search` action:
```sh
...
```
This one might be a bit too complicated for some people. Remember [flatten](http://apidock.com/ruby/Hash/flatten)? If so, use the `each` version instead.
